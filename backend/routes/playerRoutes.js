...
// GET all players with their team info (paginated, filtered, sorted)
router.get("/", async (req, res) => {
    try {
        ...
        const sortBy = req.query.sort_by || 'id'; // id, name, price, change
        ...
        // Map frontend sort keys to database columns/expressions
        ...
        if (sortBy === 'name') {
            ...
            // Calculate percentage change directly in SQL:
            // (price - reference_price) / NULLIF(reference_price, 0)
            // Note: IFNULL/COALESCE might be needed if reference_price could be absolutely null,
            // but the query below handles the zero case to prevent division by zero.
        ...
        console.log("SQL QUERY:", query, finalParams);
        const [playersSub] = await db.query(query, finalParams);

        // Map and calculate change
...
// GET all leagues and their teams
router.get("/leagues", async (req, res) => {
    try {
        ...
        results.forEach(row => {
            ...
            if (row.team_id) {
                leaguesMap[row.league_id].teams.push({
                    id: row.team_id,
                    name: row.team_name
                });
            }
...
router.get("/:id", async (req, res) => {
    try {
        ...
        // Ensure id is a number to prevent casting issues or matching other routes inadvertently
        if (isNaN(playerId)) {
